...
function sharedpanel_delete_instance($id) {
    ...
    $DB->delete_records('sharedpanel', array('id' => $sharedpanel->id));
...
/**
 * Is a given scale used by the instance of sharedpanel?
 *
 * This function returns if a scale is being used by one sharedpanel
 * if it has support for grading and scales. Commented code should be
 * modified if necessary. See forum, glossary or journal modules
 * as reference.
 *
 * @param int $sharedpanelid ID of an instance of this module
 * @return bool true if the scale is used by the given sharedpanel instance
 */
/*
function sharedpanel_scale_used($sharedpanelid, $scaleid) {
    global $DB;

    if ($scaleid and $DB->record_exists('sharedpanel', array('id' => $sharedpanelid, 'grade' => -$scaleid))) {
        return true;
    } else {
        return false;
    }
...
/**
 * Checks if scale is being used by any instance of sharedpanel.
 *
 * This is used to find out if scale used anywhere.
 *
 * @param $scaleid int
 * @return boolean true if the scale is used by any sharedpanel instance
 */
/*
function sharedpanel_scale_used_anywhere($scaleid) {
    global $DB;

    if ($scaleid and $DB->record_exists('sharedpanel', array('grade' => -$scaleid))) {
        return true;
    } else {
        return false;
    }
...
/**
 * Creates or updates grade item for the give sharedpanel instance
 *
 * Needed by grade_update_mod_grades() in lib/gradelib.php
 *
 * @param stdClass $sharedpanel instance object with extra cmidnumber and modname property
 * @param mixed optional array/object of grade(s); 'reset' means reset grades in gradebook
 * @return void
 */
/*
function sharedpanel_grade_item_update(stdClass $sharedpanel, $grades=null) {
    global $CFG;
    require_once($CFG->libdir.'/gradelib.php');

    $item = array();
    $item['itemname'] = clean_param($sharedpanel->name, PARAM_NOTAGS);
    $item['gradetype'] = GRADE_TYPE_VALUE;
    $item['grademax']  = $sharedpanel->grade;
    $item['grademin']  = 0;

    grade_update('mod/sharedpanel', $sharedpanel->course, 'mod', 'sharedpanel', $sharedpanel->id, 0, null, $item);
}
*/

/**
 * Update sharedpanel grades in the gradebook
 *
 * Needed by grade_update_mod_grades() in lib/gradelib.php
 *
 * @param stdClass $sharedpanel instance object with extra cmidnumber and modname property
 ...
 */
/*
function sharedpanel_update_grades(stdClass $sharedpanel, $userid = 0) {
    global $CFG, $DB;
    require_once($CFG->libdir.'/gradelib.php');

    $grades = array(); // Populate array of grade objects indexed by userid. @example .

    grade_update('mod/sharedpanel', $sharedpanel->course, 'mod', 'sharedpanel', $sharedpanel->id, 0, $grades);
}
*/
